...
class CustomedConfusionMatrix(ConfusionMatrix):
    ...
    """
    def __init__(self, names, save_dir, nc, conf=0.25, iou_thres=0.45, task="detect"):
    ...
    def process_batch(self, pbatch, detections, gt_bboxes, gt_cls):
        ...
        if gt_cls.shape[0] == 0:  # Check if labels is empty
            ...
            # plot empty gt
            gt_res = Results(pbatch['ori_img'], pbatch['im_file'], self.names)
...
class CustomedDetectionValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        self.confusion_matrix = CustomedConfusionMatrix(save_dir=self.save_dir, names=self.names, nc=self.nc, conf=self.args.conf)
